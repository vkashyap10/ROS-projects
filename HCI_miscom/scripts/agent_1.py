...
def callback(data):
	...
	if(data.data == "Agent 1"):

		print("Agent 1 will speak now for next 20 seconds")
...
def gotinfo(hand_sub, head_sub):
	...
	print("Agent 1 will now listen to Agent 2")
	...
	# interrupt agent
	if(action == 1):
		print("I feel there is a miscommunication!!!")
		if(mis_comm == 1):
...
if __name__ == "__main__":
	...
	rospy.init_node('agent_1', anonymous=False)

	# initialise q learning agent
	Q_agent = QLearningAgent()

	pub1 = rospy.Publisher('fluency_agent_1', String, queue_size=10)

	pub2_reward = rospy.Publisher('cumulative_reward_agent_1', Float64, queue_size=10)
	...
	# subscribe to topics
    	hand_sub = Subscriber("HTC_tracker_agent_1", htc_hand_sensor)
    	head_sub = Subscriber("head_nod_agent_1", Float64)
